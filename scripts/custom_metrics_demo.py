...
def main() -> None:
    ...
    def l1_5_distance(a: np.ndarray, b: np.ndarray) -> float:
        ...
        if a.shape != b.shape:
            raise ValueError(
                f"Input arrays must have the same shape, got {a.shape} and {b.shape}"
            )
    ...
    def mahalanobis_distance(a: np.ndarray, b: np.ndarray) -> float:
        ...
        if "cov_inv" not in globals():
            raise RuntimeError(
                "cov_inv must be defined globally before using mahalanobis_distance."
            )
        a_arr = np.array(a)
        b_arr = np.array(b)
        if a_arr.shape != b_arr.shape:
            raise ValueError(
                f"Input vectors must have the same shape, got {a_arr.shape} and {b_arr.shape}"
            )
        diff = a_arr - b_arr
        if diff.shape[0] != cov_inv.shape[0]:
            raise ValueError(
                f"Input vectors must have dimension {cov_inv.shape[0]}, got {diff.shape[0]}"
            )
    ...
    # Sample data
    data = np.array(
        [[1.0, 2.0], [3.0, 1.0], [2.0, 3.0], [4.0, 2.0], [1.5, 1.5]], dtype=np.float32
    )
    ...
    print(
        f"   Closest point using Distance.custom('l1_5'): ID {labels[0]}, distance: {distances[0]:.4f}"
    )
